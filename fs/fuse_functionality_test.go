...
func TestFUSE_FileCreation(t *testing.T) {
	...
	if mirroredPath != cacheFile {
		tu.Failf(t, "Expected mirrored path %s, got %s", cacheFile, mirroredPath)
	}

	// Verify file was created in cache
	if _, err := os.Stat(cacheFile); os.IsNotExist(err) {
		tu.Errorf(t, "File not created in cache")
	}
...
func TestFUSE_DirectoryCreation(t *testing.T) {
	...
	if mirroredPath != cacheDir {
		tu.Failf(t, "Expected mirrored path %s, got %s", cacheDir, mirroredPath)
	}

	// Verify directory was created in cache
	if stat, err := os.Stat(cacheDir); err != nil || !stat.IsDir() {
		tu.Failf(t, "Directory not created in cache or not a directory: %v", err)
	}
}
...
func TestFUSE_PathRebasingOperations(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Dir(nestedFile), 0755); err != nil {
		tu.Failf(t, "Failed to create nested directories: %v", err)
	}
	if err := os.WriteFile(nestedFile, []byte("nested content"), 0644); err != nil {
		tu.Failf(t, "Failed to create nested file: %v", err)
	}
	...
	if cachePath != expectedCachePath {
		tu.Failf(t, "RebasePathUsingCache failed: expected %s, got %s", expectedCachePath, cachePath)
	}

	// Test rebasing cache to source
	rebasedSource := ts.Root.RebasePathUsingSrc(cachePath)
	if rebasedSource != nestedFile {
		tu.Failf(t, "RebasePathUsingSrc failed: expected %s, got %s", nestedFile, rebasedSource)
	}
	...
	if !ts.Root.IsCached(nestedFile) {
		tu.Failf(t, "IsCached should return true for root node (mirrorPath is set)")
	}
...
func TestFUSE_ShadowXAttrOperations(t *testing.T) {
	...
	if !exists {
		tu.Failf(t, "Xattr should exist after setting")
	}
	if retrievedAttr.PathStatus != xattr.PathStatusDeleted {
		tu.Failf(t, "Expected PathStatusDeleted, got %d", retrievedAttr.PathStatus)
	}

	// Test IsPathDeleted function
	if !xattr.IsPathDeleted(retrievedAttr) {
		tu.Failf(t, "IsPathDeleted should return true for deleted file")
	}

	// Test with normal status
	normalAttr := &xattr.XAttr{PathStatus: xattr.PathStatusNone}
	if xattr.IsPathDeleted(*normalAttr) {
		tu.Failf(t, "IsPathDeleted should return false for normal file")
	}
}
...
func TestFUSE_WriteFileOnce(t *testing.T) {
	...
	if string(readContent) != string(content) {
		tu.Failf(t, "Content mismatch: expected %s, got %s", string(content), string(readContent))
	}

	// Test writing again (should not overwrite)
	err = rootinit.WriteFileOnce(testFile, []byte("new content"), 0644)
	if err != nil {
		tu.Failf(t, "writeFileOnce should not fail when file exists: %v", err)
	}

	// Verify content wasn't overwritten
	readContent, err = os.ReadFile(testFile)
	if err != nil {
		tu.Failf(t, "Failed to read file after second write: %v", err)
	}
	if string(readContent) != string(content) {
		tu.Failf(t, "Content should not be overwritten by writeFileOnce")
	}
}
...
func TestFUSE_FullPathOperations(t *testing.T) {
	...
	if mountPoint != ts.MountPoint {
		tu.Failf(t, "GetMountPoint failed: expected %s, got %s", ts.MountPoint, mountPoint)
	}

	// Test that the root node has the expected paths set
	if ts.Root.cachePath != ts.CacheDir {
		tu.Failf(t, "Root cachePath mismatch: expected %s, got %s", ts.CacheDir, ts.Root.cachePath)
	}
	if ts.Root.srcDir != ts.SrcDir {
		tu.Failf(t, "Root srcDir mismatch: expected %s, got %s", ts.SrcDir, ts.Root.srcDir)
	}
}
