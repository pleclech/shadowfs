...
func TestRenameTrie_SimpleRename(t *testing.T) {
	...
	if !found {
		tu.Failf(t, "Expected to find rename mapping for 'foo/baz'")
	}
	if independent {
		tu.Failf(t, "Expected path to not be independent")
	}
	if resolved != "foo/bar" {
		tu.Failf(t, "Expected resolved path 'foo/bar', got '%s'", resolved)
	}

	// Resolve nested: foo/baz/hello -> foo/bar/hello
	resolved, _, found = trie.Resolve("foo/baz/hello")
	if !found {
		tu.Failf(t, "Expected to find rename mapping for 'foo/baz/hello'")
	}
	if resolved != "foo/bar/hello" {
		tu.Failf(t, "Expected resolved path 'foo/bar/hello', got '%s'", resolved)
	}
}
...
func TestRenameTrie_NestedRename(t *testing.T) {
	...
	if !found {
		tu.Failf(t, "Expected to find rename mapping for 'foo/baz/hello/you/file.txt'")
	}
	expected := filepath.Join("foo/bar/hello/world", "file.txt")
	if resolved != expected {
		tu.Failf(t, "Expected resolved path '%s', got '%s'", expected, resolved)
	}
}
...
func TestRenameTrie_MultipleSequentialRenames(t *testing.T) {
	...
	if !found {
		tu.Failf(t, "Expected to find rename mapping for 'foo/qux'")
	}
	if resolved != "foo/baz" {
		tu.Failf(t, "Expected resolved path 'foo/baz', got '%s'", resolved)
	}

	// Resolve nested: foo/qux/hello -> foo/baz/hello
	resolved, _, found = trie.Resolve("foo/qux/hello")
	if !found {
		tu.Failf(t, "Expected to find rename mapping for 'foo/qux/hello'")
	}
	if resolved != "foo/baz/hello" {
		tu.Failf(t, "Expected resolved path 'foo/baz/hello', got '%s'", resolved)
	}
}
...
func TestRenameTrie_IndependentPath(t *testing.T) {
	...
	if !independent {
		tu.Failf(t, "Expected path to be independent")
	}
	...
	if !independent {
		tu.Failf(t, "Expected nested path to be independent")
	}
}
...
func TestRenameTrie_LongestPrefixMatch(t *testing.T) {
	...
	if !found {
		tu.Failf(t, "Expected to find rename mapping for 'foo/other'")
	}
	if resolved != "bar/other" {
		tu.Failf(t, "Expected resolved path 'bar/other', got '%s'", resolved)
	}
}
...
func TestRenameTrie_Remove(t *testing.T) {
	...
	if !found || resolved != "foo/bar" {
		tu.Failf(t, "Expected rename mapping to exist")
	}
	...
	if found {
		tu.Failf(t, "Expected rename mapping to be removed")
	}
}
...
func TestRenameTracker_MemoryFirst(t *testing.T) {
	...
	if !found {
		tu.Failf(t, "Expected to find rename mapping in memory")
	}
	if independent {
		tu.Failf(t, "Expected path to not be independent")
	}
...
func TestRenameTracker_SetIndependent(t *testing.T) {
	...
	if !found {
		tu.Failf(t, "Expected to find rename mapping")
	}
	if !independent {
		tu.Failf(t, "Expected path to be independent")
	}
