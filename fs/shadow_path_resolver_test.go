...
func TestPathResolver_ResolveSimplePath(t *testing.T) {
	...
	if errno != 0 {
		tu.Failf(t, "ResolveForRead() errno = %v, want 0", errno)
	}
	if sourcePath != testFile {
		tu.Failf(t, "ResolveForRead() sourcePath = %v, want %v", sourcePath, testFile)
	}
	expectedCachePath := filepath.Join(cacheDir, "test.txt")
	if cachePath != expectedCachePath {
		tu.Failf(t, "ResolveForRead() cachePath = %v, want %v", cachePath, expectedCachePath)
	}
	if isIndependent {
		tu.Failf(t, "ResolveForRead() isIndependent = true, want false")
	}
}
...
func TestPathResolver_ResolveRenamedPath(t *testing.T) {
	...
	if sourcePath != originalPath {
		tu.Failf(t, "ResolveForRead() sourcePath = %v, want %v", sourcePath, originalPath)
	}
	if isIndependent {
		tu.Failf(t, "ResolveForRead() isIndependent = true, want false")
	}
...
func TestPathResolver_ResolveIndependentPath(t *testing.T) {
	...
	if !isIndependent {
		tu.Failf(t, "ResolveForRead() isIndependent = false, want true")
	}
	...
	if sourcePath != expectedSourcePath {
		tu.Failf(t, "ResolveForRead() sourcePath = %v, want %v", sourcePath, expectedSourcePath)
	}
...
func TestPathResolver_ResolveDeletedPath(t *testing.T) {
	...
	if err := os.WriteFile(deletedPath, []byte(""), 0644); err != nil {
		tu.Failf(t, "Failed to create deletion marker: %v", err)
	}
	// Mark as deleted
	attr := xattr.XAttr{
	...
	if errno := xattr.Set(deletedPath, &attr); errno != 0 {
		tu.Failf(t, "Failed to set deletion xattr: %v", errno)
	}
	...
	if errno != syscall.ENOENT {
		tu.Failf(t, "ResolveForRead() errno = %v, want ENOENT", errno)
	}
}
...
func TestPathResolver_DeletionClearsRenameMapping(t *testing.T) {
	...
	if !found {
		tu.Failf(t, "Rename mapping should exist before deletion")
	}
	...
	_, _, found = renameTracker.ResolveRenamedPath("foo/baz")
	if found {
		tu.Failf(t, "Rename mapping should be removed after deletion")
	}
}
...
func TestPathResolver_RecreationAfterDeletionIsIndependent(t *testing.T) {
	...
	if errno := xattr.Remove(deletedCachePath); errno != 0 && errno != syscall.ENODATA {
		tu.Failf(t, "Failed to remove deletion xattr: %v", errno)
	}
	...
	if !isIndependent {
		tu.Failf(t, "ResolveForRead() isIndependent = false, want true (recreated path should be independent)")
	}
	// Should use mount point path, not resolved source path
	expectedSourcePath := filepath.Join(srcDir, "foo", "baz", "test.txt")
	if sourcePath != expectedSourcePath {
		tu.Failf(t, "ResolveForRead() sourcePath = %v, want %v (independent path should not resolve)", sourcePath, expectedSourcePath)
	}
}
...
func TestPathResolver_ResolveRootPath(t *testing.T) {
	...
	if cachePath != cacheDir {
		tu.Failf(t, "ResolveForRead() cachePath = %v, want %v", cachePath, cacheDir)
	}
	if sourcePath != srcDir {
		tu.Failf(t, "ResolveForRead() sourcePath = %v, want %v", sourcePath, srcDir)
	}
	if isIndependent {
		tu.Failf(t, "ResolveForRead() isIndependent = true, want false")
	}
...
func TestPathResolver_ResolveForWrite(t *testing.T) {
	...
	if errno != 0 {
		tu.Failf(t, "ResolveForWrite() errno = %v, want 0", errno)
	}
	// Source path should be resolved (for reference)
	expectedSourcePath := filepath.Join(srcDir, "foo", "bar", "test.txt")
	if sourcePath != expectedSourcePath {
		tu.Failf(t, "ResolveForWrite() sourcePath = %v, want %v", sourcePath, expectedSourcePath)
	}
}
